...
class CMT(object):
    ...
    def initialise(self, im_gray0, tl, br):
        ...
        ind = in_rect(keypoints_cv, tl, br)
        ...
        selected_keypoints = keypoints_cv_to_np(selected_keypoints_cv)
        ...
        _ = keypoints_cv_to_np(background_keypoints_cv)
    ...
    def estimate(self, keypoints):
        ...
        if keypoints.size > 1:
            ...
            if not all(duplicate_classes):
                ...
                dists = L2norm(pts_allcombs0 - pts_allcombs1)
                ...
                votes = keypoints[:, :2] - scale_estimate * (rotate(self.springs[keypoint_class - 1], med_rot))
    ...
    def track(self, _, im_gray, roi_box=None):

        if roi_box:
            # print("initializing")
            # print(roi_box[:2], roi_box[2:])
            self.initialise(im_gray, roi_box[:2], roi_box[2:])
            return

        tracked_keypoints, _ = track(self.im_prev, im_gray, self.active_keypoints)
        ...
        if len(keypoints_cv) > 0:
            transformed_springs = scale_estimate * rotate(self.springs, -rotation_estimate)
            for i in range(len(keypoints_cv)):
                ...
                if not any(isnan(center)):
                    ...
                    displacements = L2norm(transformed_springs - relative_location)
        ...
        if not any(isnan(self.center)) and self.active_keypoints.shape[0] > self.num_initial_keypoints / 10:
            ...
            tl = array_to_int_tuple(
                center + scale_estimate * rotate(self.center_to_tl[None, :], rotation_estimate).squeeze())
            tr = array_to_int_tuple(
                center + scale_estimate * rotate(self.center_to_tr[None, :], rotation_estimate).squeeze())
            br = array_to_int_tuple(
                center + scale_estimate * rotate(self.center_to_br[None, :], rotation_estimate).squeeze())
            bl = array_to_int_tuple(
                center + scale_estimate * rotate(self.center_to_bl[None, :], rotation_estimate).squeeze())
